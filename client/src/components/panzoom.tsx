/* World Cup HQ — PanZoom: a fixed-size content canvas that fits its viewport,
   ...
   Ported byte-for-byte from the prototype's home/panzoom.jsx. */
...
export type PanZoomHandle = {
  focusTo: (cx: number, cy: number, k?: number) => void;
  fit: () => void;
};
...
export function PanZoom({
  ...
  maxScale?: number;
  padding?: number;
  onReady?: (handle: PanZoomHandle) => void;
  /* When true, the canvas uses touch-action:pan-y at base zoom (so a single-finger
     swipe scrolls the page) and switches to none once zoomed in (so dragging pans).
     Default keeps the bracket behaviour: touch-action:none always. */
  touchPanY?: boolean;
  children?: ReactNode;
}) {
  ...
  const pinch = useRef<{ dist0: number; k0: number; c: { x: number; y: number } } | null>(null);

  const clampK = (k: number) => Math.max(minScale, Math.min(maxScale, k));
  ...
  const focusTo = useCallback((cx: number, cy: number, k = 1.7) => {
    const vp = vpRef.current; if (!vp) return;
    const kk = clampK(k);
    setTf({ k: kk, x: vp.clientWidth / 2 - cx * kk, y: vp.clientHeight / 2 - cy * kk });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [minScale, maxScale]);
  useEffect(() => { if (onReady) onReady({ focusTo, fit }); }, [onReady, focusTo, fit]);
  useEffect(() => {
    const onR = () => fit();
  ...
  const zoomAt = (factor: number, sx: number, sy: number) => setTf((t) => {
  ...
  const onPointerDown = (e: ReactPointerEvent) => {
    // README fix: ignore pointerdowns that start on (or inside) a <button> so the
    // +/-/fit controls are not swallowed by the canvas pointer-capture.
  ...
  const btn: CSSProperties = {
    width: 38, height: 38, borderRadius: 11, border: '3px solid var(--ink)',
  ...
  return (
    <div ref={vpRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove}
      ...
      style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden',
        touchAction: touchPanY ? (tf.k > 1 ? 'none' : 'pan-y') : 'none',
        cursor: pan.current ? 'grabbing' : 'grab', userSelect: 'none' }}>
