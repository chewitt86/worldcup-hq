...
/* A single game on the board. Fields are optional because matches may be
   upcoming (no score/winner) or in the group stage (no host/city/date). */
...
function MatchPopup({
  ...
  const m = match;
  const hostName =
    ({ USA: 'USA', CAN: 'Canada', MEX: 'Mexico' } as Record<string, string>)[
      m.host ?? ''
    ] || m.host;

  const Row = ({ code, score }: { code: string; score: string | number }) => {
    ...
    return (
      <div
        ...
        style={{
          ...
          background: win ? 'var(--sun)' : 'var(--cream)',
          opacity: dead ? 0.65 : 1,
        }}
      >
          ...
          >
            {owners.length ? (
              owners.map((p) => (
                ...
                >
                  <Avatar person={p} size={16} ring={false} />
                  {p.name}
                </span>
              ))
            ) : (
              <span style={{ fontSize: 11, fontWeight: 700, opacity: 0.5 }}>
        ...
        {!m.played && win && (
          <span className="head" style={{ fontSize: 12 }}>
            WINS ✓
          </span>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {m.played && m.score ? `${m.score[0]} – ${m.score[1]}` : 'VS'}
        </div>
            ...
            {m.date && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 14 }}>
                  {m.date} · {m.time}
                  <span style={{ color: 'var(--tomato)', fontFamily: 'var(--head)' }}> BST</span>
                </span>
              </div>
            )}
            {m.city && (
