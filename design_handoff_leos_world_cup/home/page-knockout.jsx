...
function koDead(code) { return window.WCHQ.ELIMINATED.includes(code); }

/* bracket connector lines (geometry matches the column layout below) */
function KOConnectors() {
  const centers = (N) => Array.from({ length: N }, (_, i) => 50 + (i + 0.5) * (530 / N));
  const segs = [];
  const pair = (cE, cYs, pE, pYs) => {
    const midX = (cE + pE) / 2;
    pYs.forEach((py, k) => {
...
function TieCard({ tie, stage, gi, onGame, people, big }) {
  const W = window.WCHQ;
  const g = W.koGame(stage, gi);
  const played = g.played && g.score;
  const actualWin = played ? (g.score[0] > g.score[1] ? tie.a : g.score[1] > g.score[0] ? tie.b : tie.w) : tie.w;
  return (
        ...
        return (
          <div key={code} style={{ display: "flex", alignItems: "center", gap: 6, padding: big ? "9px 11px" : "6px 8px",
            ...
            <span className="head" style={{ fontSize: big ? 16 : 13.5, color: win ? "var(--ink)" : "#d4ddf2" }}>
              {big ? W.TEAMS[code].name : code}</span>
            <span style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
              <OwnerIcons code={code} people={people} size={big ? 20 : 15} />
              {played
                ? <span className="head" style={{ fontSize: big ? 16 : 13, color: win ? "var(--ink)" : "#d4ddf2" }}>{g.score[i]}</span>
...
function RoundCol({ label, ties, stage, offset = 0, onGame, people, width }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", width, flex: "0 0 auto", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-around", gap: 8 }}>
        {ties.map((t, i) => <TieCard key={i} tie={t} stage={stage} gi={offset + i} onGame={onGame} people={people} />)}
      </div>
    </div>
  );
}

function CenterCol({ final, onGame, people }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", width: 178, flex: "0 0 auto", position: "relative", zIndex: 1,
      ...
      <TieCard tie={final} stage="F" gi={0} onGame={onGame} people={people} big />
...
/* ---- game detail popup ---- */
function GameModal({ game, onClose, onTeam, people }) {
  const W = window.WCHQ;
  const { tie, stage, i } = game;
  const info = W.koGame(stage, i);
  const host = { USA: "USA", CAN: "Canada", MEX: "Mexico" }[info.host];
  const played = info.played && info.score;
  const actualWin = played ? (info.score[0] > info.score[1] ? tie.a : info.score[1] > info.score[0] ? tie.b : tie.w) : tie.w;
    ...
    <div onClick={onClose} style={{ position: "absolute", inset: 0, zIndex: 80, background: "rgba(27,42,74,.5)",
      ...
      <div onClick={(e) => e.stopPropagation()} className="sticker"
        ...
        <div className="head" style={{ textAlign: "center", color: "var(--tomato)", fontSize: 16, margin: "6px 0" }}>
          {info.played ? `${info.score[0]} – ${info.score[1]}` : "VS"}</div>
        <TeamRow code={tie.b} />
        ...
        <div style={{ marginTop: 14, background: "var(--cream2)", border: "3px solid var(--ink)", borderRadius: 14,
          padding: "11px 13px" }}>
          {info.played ? (
            <div className="head" style={{ fontSize: 15, textAlign: "center" }}>FULL-TIME · {info.score[0]}–{info.score[1]}</div>
          ) : (
        ...
        {!info.played && (
          <div style={{ textAlign: "center", fontWeight: 700, fontSize: 12, color: "var(--ink-soft)", marginTop: 10 }}>
            ✨ Projected winner: <span className="head">{W.TEAMS[tie.w].name}</span>
          </div>
        )}
      </div>
    </div>
...
function ChampionHero({ champ, runnerUp, onTeam }) {
  const W = window.WCHQ; const t = W.TEAMS[champ];
  return (
    <div className="sticker" style={{ position: "relative", padding: "16px 18px", overflow: "hidden",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ color: "#9fb2d4", fontWeight: 700, fontSize: 13, marginTop: 6 }}>
          Beats {W.TEAMS[runnerUp]?.name} in the final ✨</div>
      </div>
  ...
  return (
    <div className="sticker" style={{ padding: 16, background: "#222b3d" }}>
      <div className="head" style={{ color: "var(--cream)", fontSize: 17, letterSpacing: ".5px",
        ...
        🪦 THE GRAVEYARD
        <span style={{ marginLeft: "auto", fontSize: 11, color: "#8aa0c8" }}>{W.ELIMINATED.length} teams out</span>
      </div>
...
function KnockoutPage() {
  ...
  // re-seed when odds change (bracketNonce bumps); results also re-render
  const b = React.useMemo(() => W.bracketFull(), [store.bracketNonce]);
  const runnerUp = b.final.a === b.champ ? b.final.b : b.final.a;
  const apiRef = React.useRef(null);
  const onReady = React.useCallback((api) => { apiRef.current = api; }, []);
  ...
  const teamOpts = Object.keys(W.TEAMS).sort((x, y) => W.TEAMS[x].name.localeCompare(W.TEAMS[y].name));
  ...
  const wA = 120, wB = 128;
  const openTeam = (code) => { setGame(null); setTeam(code); };
  ...
  const bracket = (
    <div style={{ width: KO_CONTENT_W, height: KO_CONTENT_H, display: "flex", alignItems: "stretch",
      ...
      <KOConnectors />
      <RoundCol label="ROUND OF 32" ties={r32L} stage="R32" offset={0} onGame={setGame} people={people} width={wA} />
      <RoundCol label="LAST 16" ties={r16L} stage="R16" offset={0} onGame={setGame} people={people} width={wA} />
      <RoundCol label="QUARTERS" ties={qfL} stage="QF" offset={0} onGame={setGame} people={people} width={wB} />
      <RoundCol label="SEMIS" ties={sfL} stage="SF" offset={0} onGame={setGame} people={people} width={wB} />
      <CenterCol final={b.final} onGame={setGame} people={people} />
      <RoundCol label="SEMIS" ties={sfR} stage="SF" offset={1} onGame={setGame} people={people} width={wB} />
      <RoundCol label="QUARTERS" ties={qfR} stage="QF" offset={2} onGame={setGame} people={people} width={wB} />
      <RoundCol label="LAST 16" ties={r16R} stage="R16" offset={4} onGame={setGame} people={people} width={wA} />
      <RoundCol label="ROUND OF 32" ties={r32R} stage="R32" offset={8} onGame={setGame} people={people} width={wA} />
    </div>
  );
  ...
  return (
    <React.Fragment>
      ...
      <ChampionHero champ={b.champ} runnerUp={runnerUp} onTeam={setTeam} />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <select value="" onChange={(e) => { if (e.target.value) jump(e.target.value); }}
          ...
          {teamOpts.map((c) => (
            <option key={c} value={c}>{W.backers(c, people).length ? "⭐ " : ""}{W.TEAMS[c].name}</option>
          ))}
        </select>
      ...
      <Graveyard onTeam={setTeam} />

      {game && <GameModal game={game} people={people} onClose={() => setGame(null)} onTeam={openTeam} />}
      {team && <TeamPopup code={team} onClose={() => setTeam(null)} onPerson={app.openPerson} />}
    </React.Fragment>
  );
}

Object.assign(window, { KnockoutPage, TieCard, RoundCol, CenterCol, GameModal, ChampionHero, Graveyard, OwnerIcons });
