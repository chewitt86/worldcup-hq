...
function TeamCard({ code, onOpen, people, wide }) {
  const t = window.WCHQ.TEAMS[code];
  const dead = window.WCHQ.ELIMINATED.includes(code);
  return (
...
function TeamPopup({ code, onClose, onPerson }) {
  if (!code) return null;
  const W = window.WCHQ;
  const t = W.TEAMS[code];
  const g = W.groupOf(code);
  const st = W.STANDINGS[code];
  const dead = W.ELIMINATED.includes(code);
  const results = (W.GROUP_RESULTS[g] || []).filter((r) => r.a === code || r.b === code);
      ...
      <div onClick={(e) => e.stopPropagation()} className="sticker noscroll"
        ...
        <div style={{ padding: 18 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[["Played", st.p], ["Won", st.w], ["Drawn", st.d], ["Lost", st.l], ["Pts", st.pts]].map(([k, v], i) => (
...
function TeamsPage() {
  ...
  const codes = Object.keys(W.TEAMS).filter((c) => tier === "All" || W.TEAMS[c].tier === tier)
    .sort((a, b) => W.oddsNum(a) - W.oddsNum(b));

  return (
    <React.Fragment>
      <PageTitle sub={`${Object.keys(W.TEAMS).length} nations · ${W.ELIMINATED.length} already out`}>TEAMS</PageTitle>
...
Object.assign(window, { TeamsPage, TeamCard, TeamPopup });
