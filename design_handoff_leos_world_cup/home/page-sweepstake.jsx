...
function PersonCard({ p, rank, onPerson, wide }) {
  ...
  const rankCol = { 1: "var(--sun)", 2: "#d7dde6", 3: "#f0b173" }[rank];
...
function SweepstakePage() {
  const app = useApp();
  const { people } = useStoreState();
  const wide = app.wide;
  const ranked = React.useMemo(() => [...people].sort((a, b) => b.points - a.points), [people]);
...
Object.assign(window, { SweepstakePage, PersonCard, isEliminated });
